// Package cli assembles the hupu command tree from the hupu
...
// NewApp assembles the kit application from the hupu domain. The
...
//
// To add a command, declare it in hupu/domain.go with kit.Handle and it
// appears here automatically. Reach for app.AddCommand only for a verb that does
// not fit the emit-records shape, the way version does below.
func NewApp() *kit.App {
	id := hupu.Domain{}.Info().Identity
	id.Version = Version

	app := kit.New(id)
	(hupu.Domain{}).Register(app)
