// Package cli builds the hupu command tree on top of the hupu library.
package cli
...
import (
	...
	"github.com/tamnd/hupu-cli/pkg/render"
)
...
func Root() *cobra.Command {
	app := &App{cfg: hupu.DefaultConfig()}

	root := &cobra.Command{
		Use:   "hupu",
		Short: "Browse Hupu sports community posts (虎扑)",
		Long: `hupu reads the Hupu BBS public homepage and returns trending posts.
No API key or login is required. It returns records as table, JSON, JSONL,
CSV, TSV, or URLs.

hupu is an independent tool and is not affiliated with Hupu (虎扑).`,
		SilenceUsage:  true,
		SilenceErrors: true,
