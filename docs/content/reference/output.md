...
describes what users get. Pick a format with `-o`, or let hupu choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
hupu <command> -o table   # aligned columns for reading
hupu <command> -o jsonl   # one JSON object per line, for piping
hupu <command> -o json    # a single JSON array
hupu <command> -o csv     # spreadsheet friendly
hupu <command> -o tsv     # tab-separated
hupu <command> -o url     # just the URL column
hupu <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
hupu <command> --fields id,title,url
```
...
```bash
hupu <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
hupu <command>            # a table, because this is a terminal
hupu <command> | wc -l    # JSONL, because this is a pipe
```
