...
hupu is a single binary. It speaks to hupu over plain HTTPS,
shapes the responses into clean records, and gets out of your way. There is
nothing to sign up for and nothing to run alongside it.

## How it is built

- A **library package** (`hupu`) holds the HTTP client and the typed
  data models. It paces requests, sets an honest User-Agent, and retries the
  transient failures any public site throws under load.
- A **domain** (`hupu/domain.go`) declares each operation once on the
...
- A thin **`cmd/hupu`** hands the assembled app to `kit.Run`, which
...
```bash
hupu page <path>                  # the command
hupu serve --addr :7777           # GET /v1/page/<path>
hupu mcp                          # the page tool, over stdio
ant get hupu://page/<path>        # the URI dereference (via a host)
```
...
hupu is a read-only client over data hupu already serves
